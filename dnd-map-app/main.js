...
import { townStyles, townGJs } from './provinceTowns';
...
var citiesLayer = new VectorLayer({
  source: new VectorSource({
    features: new GeoJSON().readFeatures(townGJs),
  }),
  style: townStyles,
  minZoom: 5,
});
...
distanceInput.addEventListener('input', function (event) {
  ...
  const currentCoordinate = ref_q_route.getCoordinateAt(d > 1 ? 2 - d : d);
